...
use utils::jupiter2::D9;
use utils::*;

#[tokio::test]
async fn jupiter_quotes2() {
    // Setup
    ...
    let mut jupiter_quote_result =
        jupiter2::Gamma::from_keyed_account(&keyed_account, &amm_context).unwrap();
    ...
    // Set params for oracle based swaps
    {
        ...
        test_env
            .update_pool(
                &admin, pool_id, amm_index, 10, // 1_000_000
                3,
            )
            .await;
    }
        ...
        if i % 3 == 0 {
            ...
            test_env
                .oracle_price_update(
                    ...
                    pool_id,
                    amm_index,
                    pool_state.token_0_vault_amount as u128 * D9
                        / pool_state.token_1_vault_amount as u128,
                )
                .await;
        }
        let pool_state_info = test_env.get_account_info(pool_id).await.unwrap().unwrap();
